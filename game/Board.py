...
class Board:
    def __init__(self, p1_x=BOARDSIZEMID, p1_y=BOARDSIZE, p2_x=BOARDSIZEMID, p2_y=0):
        ...
        self.wh = WallHandler()
        self.fh = FieldHandler(p1_x, p1_y, p2_x, p2_y)
        self.ph = PlayerHandler(p1_x, p1_y, p2_x, p2_y)
    ...
    def setHorizontalWall(self, x, y, val=True):
        ...
        if x != 0:
            self.wh.incrWallcenterCount(x - 1, y, increment)
        if x != BOARDSIZE:
            self.wh.incrWallcenterCount(x, y, increment)
    ...
    def setVerticalWall(self, x, y, val=True):
        ...
        increment = 1 if val else -1
        if y != 0:
            self.wh.incrWallcenterCount(x, y - 1, increment)
        if y != BOARDSIZE:
            self.wh.incrWallcenterCount(x, y, increment)
    ...
    def getPlayerMoves(self, state):
        ...
        for d in [Dir.N, Dir.E, Dir.S, Dir.W]:
            ...
            if pos_moved != other_pos:
                ...
                if self.fh.getWall(x_moved, y_moved, d) or not self.fh.getDir(
                    x_moved, y_moved, d
                ):  # Side Jump
                    for d2 in [Dir.N, Dir.E, Dir.S, Dir.W]:
                        if d2 in [d, mir(d)]:
                            continue
                        if self.fh.getWall(x_moved, y_moved, d2):
                            continue
    ...
    def findPaths(self, x, y, p):
        ...
        for d in nexts:
            ...
            if d in [Dir.N, Dir.S]:
                self.wh.setWallOnPath(nw_x, nw_y, Orientation.H, p)
            if d in [Dir.E, Dir.W]:
    ...
    def isWallSetable(self, x, y, o):
        ...
        if midCon_count == 1 and (
            self.wh.isWallSet(x, y, o) or self.wh.isWallSet(w2_x, w2_y, o)
        ):
            return False

        # if there are 2 or more loose connectors
        if self.wh.countFreeCentersInWall(x, y, o) in [2, 3]:
            return True
