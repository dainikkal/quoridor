...
class Field:
    def __init__(self, x, y):
        ...
        self.nexts = [[Dir.N] if y != 0 else [], [Dir.S] if y != BOARDSIZE else []]

        self.prevs = [[Dir.S] if y != BOARDSIZE else [], [Dir.N] if y != 0 else []]
        self.disconnected = [False, False]

    def setPlayer(self, p=Player.Empty):
        self.player = p
    ...
    def getE(self):
        return (self.x + 1, self.y) if not self.Walls[Dir.E] else self.getPos()

    def getS(self):
        return (self.x, self.y + 1) if not self.Walls[Dir.S] else self.getPos()
    ...
    def getWallDir(self, d):
        ...
        """
        if d == Dir.N:
            return self.getN() if self.y != 0 else None
        if d == Dir.E:
            return self.getPos() if self.x != BOARDSIZE else None
        if d == Dir.S:
            return self.getPos() if self.y != BOARDSIZE else None
        if d == Dir.W:
            return self.getW() if self.x != 0 else None
    ...
    def getDir(self, d):
        ...
        if d == Dir.N:
            return self.getN() if self.y != 0 else None
        if d == Dir.E:
            return self.getE() if self.x != BOARDSIZE else None
        if d == Dir.S:
            return self.getS() if self.y != BOARDSIZE else None
        if d == Dir.W:
            return self.getW() if self.x != 0 else None
    ...
    def setWall(self, d, val=True):
        self.Walls[d] = val

    def getHeuristic(self, p):
        return self.heuristic[p]

    def setHeuristic(self, p, val):
        self.heuristic[p] = val
    ...
    def getDisconnected(self, p):
        return self.disconnected[p]

    def setDisconnected(self, p, val):
        self.disconnected[p] = val
