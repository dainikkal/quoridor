...
class FieldHandler:
    def __init__(self, p1_x=BOARDSIZEMID, p1_y=BOARDSIZE, p2_x=BOARDSIZEMID, p2_y=0):
        ...
        self.fields[(p1_x, p1_y)].setPlayer(Player.P1)
        self.fields[(p2_x, p2_y)].setPlayer(Player.P2)
        self.disconnected = [[], []]

        self.help_checkedForPlayer = []

    def getWall(self, x, y, d):
        return self.fields[(x, y)].getWall(d)

    def setWall(self, x, y, d, val=True):
        self.fields[(x, y)].setWall(d, val)

    def getNexts(self, x, y, p):
        return self.fields[(x, y)].getNexts(p)

    def getDir(self, x, y, d):
        return self.fields[(x, y)].getDir(d)

    def getHeuristic(self, x, y, p):
        return self.fields[(x, y)].getHeuristic(p)
    ...
    def findVisitiableNeighbours(self, x, y, p):
        ...
        for d in range(4):
            ...
            if posD == None:
                continue
            # Discnonnected
            if self.fields[posD].getDisconnected(p):
                continue
    ...
    def doesPlayerReachGoal(self, x, y, p):
        ...
        if self.fields[(x, y)].getHeuristic(p) == 0:
            return True
        ...
        for d in [Dir.N, Dir.E, Dir.S, Dir.W]:
            ...
            if n_pos and n_pos not in self.help_checkedForPlayer:
                n_x, n_y = n_pos
                if self.doesPlayerReachGoal(n_x, n_y, p):
                    return True
