...
class WallHandler:
    def __init__(self):
        ...
        self.wallcenter = {
            (x, y): 0 for x in range(BOARDSIZE) for y in range(BOARDSIZE)
        }
    ...
    def isCenterSet(self, x, y):
        return False if self.wallcenter[(x, y)] < 2 else True

    def getWallcenterCount(self, x, y):
        return self.wallcenter[(x, y)]

    def incrWallcenterCount(self, x, y, increment=1):
        self.wallcenter[(x, y)] += increment
    ...
    def isWallOnPath(self, x, y, o, p):
        return self.wallsOnPath[(x, y, o, p)]
